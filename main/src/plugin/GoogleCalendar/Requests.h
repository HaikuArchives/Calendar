...
public:
	...
	virtual void 	DebugMessage(
		BUrlRequest* caller, BUrlProtocolDebugMessage type, const char* text)
	{
		if (fTraceLogging)
			printf("jrpc: %s\n", text);
	}

	void 			SetDownloadIO(BDataIO* downloadIO) { fDownloadIO = downloadIO; }
...
public:
		...
		BMessage& responseMessage)
	{
		...
		BUrl link(url);
		BUrlRequest* request
			= BUrlProtocolRoster::MakeRequest(link, NULL, &listener);
		...
		if (jsonString != NULL) {
			BMemoryIO* data
				= new BMemoryIO(jsonString->String(), jsonString->Length() - 1);
