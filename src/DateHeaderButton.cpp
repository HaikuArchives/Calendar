...
DateHeaderButton::DateHeaderButton()
	:
	BControl("DateHeaderButton", NULL, new BMessage(kSetCalendarToCurrentDate),
		B_WILL_DRAW)
{
	fMouseInView = false;
	fMouseDown = false;
...
DateHeaderButton::Draw(BRect updateRect)
{
	...
	if (fMouseInView == true && IsEnabled() == true) {
		...
		be_control_look->DrawButtonFrame(
			this, buttonBound, updateRect, LowColor(), ViewColor(), flags);
		be_control_look->DrawButtonBackground(
			this, buttonBound, updateRect, LowColor(), flags);
	...
	float monthWeekOffset_x
		= dayOffset_x + font.StringWidth(fDayText) + spacing;
...
DateHeaderButton::MouseMoved(BPoint where, uint32 code, const BMessage* drag)
{
	...
	if (inView != fMouseInView) {
		...
		if (fMouseDown == true && Window() != NULL
			...
			fMouseDown
				= (Window()->CurrentMessage()->FindInt32("buttons") != 0);
...
DateHeaderButton::_UpdateDateHeader()
{
	...
	dateFormat.Format(dateString, fieldPositions, positionCount, timeValue,
		B_FULL_DATE_FORMAT);
